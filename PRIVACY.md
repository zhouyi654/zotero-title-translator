# 隐私说明

## 插件处理的数据

插件读取 Zotero 条目的标题；当用户执行摘要翻译或开启摘要自动翻译时，也会读取摘要。需要翻译的标题或摘要会发送给用户选择的翻译服务。

插件不会主动发送：

- PDF 文件；
- 作者信息；
- 笔记；
- 批注；
- 整个 Zotero 数据库；
- Zotero 账号凭据。

## 本地方案

当 Ollama 或 LibreTranslate 在本机运行，并将地址设置为 `localhost` 时，标题可以留在当前设备。

## 远程服务

选择远程服务时，待翻译的标题或摘要会传输给相应服务商。服务商自身的隐私政策、数据保留规则、地区处理和账户设置适用。

## 译题保存

标题和摘要译文保存到 Zotero 条目的 `Extra` 字段：

```text
titleTranslation: 中文译题
abstractTranslation: 中文摘要译文
```

旧版 `ZoteroTitleTranslation` 会在升级后迁移到 `titleTranslation`。

该内容可能随 Zotero 条目同步。

## API Key

API Key 当前保存在本机 Zotero 首选项中，没有由插件进行额外加密。

密钥不会发送给本项目作者，但可访问本地 Zotero profile 的软件或人员可能读取密钥。

建议使用权限受限、可撤销并设置额度上限的 API Key。

## 术语表

自定义术语表保存在本机 Zotero 首选项中。命中的术语映射会随待翻译标题或摘要一起写入远程翻译请求；本机服务仍只在本机处理。


## PDF2zh 术语桥接

启用桥接后，本插件会把当前术语的“源术语、标准译法和目标语言”写入 PDF2zh Server 本地文件夹，并修改其本地 `config.toml`。错误译法别名不会写入 PDF2zh CSV。

桥接本身不上传 PDF 或术语；后续数据传输由用户选择的 PDF2zh 翻译服务决定。
