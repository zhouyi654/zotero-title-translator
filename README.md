...
```text
dist/zotero-title-translator-0.3.4.xpi
```
...
修复了分类右键翻译错误扫描整个文献库的问题。

现在的范围规则：

- 右键实际分类：只翻译该分类直接包含的普通文献；
- 不自动递归翻译子分类；
- 右键“我的文库”或群组文库根节点：翻译整个文献库；
- 工具菜单根据当前选中节点自动判断分类或文献库范围；
- 开始前的确认框会明确显示“分类”或“文献库”及条目数量。

同一条文献可以同时属于多个分类。插件更新的是文献条目本身的译题，
因此翻译后该条目在其他分类中也会显示相同译题。
...
该开关默认关闭。启用后，插件通过 Zotero 条目通知系统监听新增加的普通文献条目，
并在等待指定秒数后自动翻译标题。

可触发的常见场景：

- Zotero Connector 保存网页文献；
- RIS、BibTeX、CSL JSON 等格式导入；
- 通过 DOI、PMID、ISBN 添加条目；
- PDF 元数据识别后生成父级文献；
- 从其他设备同步到本机的新文献；
- 其他会在 Zotero 中创建普通文献条目的操作。
...
- 跳过附件、笔记和批注；
- 跳过不可编辑条目；
- 仍使用当前选择的翻译服务、并发、限流和重试设置；
- 自动翻译失败不会阻止文献导入。

“导入后等待时间”默认为 3 秒，用于等待 Zotero 写入完整标题和其他元数据。
该功能只处理开关启用后新增的条目，不会自动扫描已有文献库。
